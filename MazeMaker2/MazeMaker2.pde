...
static int dimensionInitial = 50; //size of each square edge
static int offsetX = 15,offsetY = 15;//for presenting purposes
static float  scl = 1;//for magnifying
/* TASKS

allow changing inner/outer through dragging
...
float getMouseX()
{
  return mouseX/dimension;
}

float getMouseY()
{
  return mouseY/dimension;
}
void mouseWheel(MouseEvent event) {
  float e = event.getCount();
   ...
   if( scl < 1)
        scl = 1;
  println(scl); 
  dimension = (int)(dimensionInitial/scl);
  scaleLines();
}
static void scaleLines()
{
   for(int i =0 ; i < lines.size();i++)
   {
     lines.get(i).x1  /= dimension;
     lines.get(i).x2  /= dimension;
     lines.get(i).y1  /= dimension;
     lines.get(i).y2  /= dimension;
     
     /*
     lines.get(i).x2 *= ( dimensionInitial/dimension);
     lines.get(i).x2 *= ( dimensionInitial/dimension);
     lines.get(i).y1 *= ( dimensionInitial/dimension);
     lines.get(i).y2 *= ( dimensionInitial/dimension);
