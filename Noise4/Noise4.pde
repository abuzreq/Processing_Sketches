...
void setup()
{
  ...
  //this will reduce the number of the randoms given above by one (due to the binary nature of the operaions applied i.e. max , min , avg)
    ...
    //#Main idea here is that we accumelate the noise by additon of taking the difference
    for (int m = 0; m < strings.length; m++)
    {
      ...
      for (int i = 1; i < strings[m].length(); i++)
      {
        ...
        newNoise +=  accumNoise + n - 48; // turn from char to int
        accumNoise += int(newNoise);
        //   println(n);
      }
      strings[m] = newNoise;
    }
